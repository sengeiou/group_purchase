...
/**
 * The type Activity v 12 vo.
 *
 * @author pavawi
 */
@EqualsAndHashCode(callSuper = true)
@Data
public class ActivityV12Vo extends ActivityVo {
    ...
    private List<ActivityGoodsV12Vo> actGoodsV12List;
    ...
    public List<ActivityGoods> getActGoodsList() {
        if (this.actGoodsV12List == null || this.actGoodsV12List.size() <= 0) {
            throw new GlobalException(CodeMsg.BIND_ERROR.fillArgs("活动商品列表不能为空"));
        }
        List<ActivityGoods> activityGoodsList = new ArrayList<>();
        int i = 1;
        for (ActivityGoodsV12Vo activityGoodsVo : this.actGoodsV12List) {
